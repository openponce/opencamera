...
public class FlutterFFmpegGetMediaInformationAsyncTask extends AsyncTask<String, Integer, MediaInformation> {
    ...
    protected MediaInformation doInBackground(final String... strings) {
        ...
        if ((strings != null) && (strings.length > 0)) {
            ...
            if (timeout == null) {
                Log.d(OpenCameraPlugin.LIBRARY_NAME, String.format("Getting media information for %s", path));
                mediaInformation = FFmpeg.getMediaInformation(path);
            } else {
                Log.d(OpenCameraPlugin.LIBRARY_NAME, String.format("Getting media information for %s with timeout %d.", path, timeout.longValue()));
    ...
    protected void onPostExecute(final MediaInformation mediaInformation) {
        flutterFFmpegResultHandler.success(result, OpenCameraPlugin.toMediaInformationMap(mediaInformation));
    }
