...
public class FlutterFFmpegExecuteAsyncArgumentsTask extends AsyncTask<String, Integer, Integer> {
    ...
    protected Integer doInBackground(final String... dummyString) {
        ...
        Log.d(OpenCameraPlugin.LIBRARY_NAME, String.format("Running FFmpeg with arguments: %s.", Arrays.toString(argumentsArray)));

        int rc = FFmpeg.execute(argumentsArray);

        Log.d(OpenCameraPlugin.LIBRARY_NAME, String.format("FFmpeg exited with rc: %d", rc));
    ...
    protected void onPostExecute(final Integer rc) {
        flutterFFmpegResultHandler.success(result, OpenCameraPlugin.toIntMap(OpenCameraPlugin.KEY_RC, rc));
    }
