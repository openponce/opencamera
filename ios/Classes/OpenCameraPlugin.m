...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    OpenCameraPlugin* instance = [[OpenCameraPlugin alloc] init];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([@"getPlatform" isEqualToString:call.method]) {
        ...
        result([OpenCameraPlugin toStringDictionary:KEY_PLATFORM :[NSString stringWithFormat:@"%@-%@", PLATFORM_NAME, architecture]]);

    } else if ([@"getFFmpegVersion" isEqualToString:call.method]) {

        result([OpenCameraPlugin toStringDictionary:KEY_VERSION :[MobileFFmpeg getFFmpegVersion]]);
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            result([OpenCameraPlugin toIntDictionary:KEY_RC :[NSNumber numberWithInt:rc]]);
        });
        ...
        result([OpenCameraPlugin toIntDictionary:KEY_LOG_LEVEL :[NSNumber numberWithInt:logLevel]]);
        ...
        result([OpenCameraPlugin toStatisticsDictionary:statistics]);
        ...
        result([OpenCameraPlugin toStringDictionary:KEY_PACKAGE_NAME :packageName]);
        ...
        result([OpenCameraPlugin toIntDictionary:KEY_LAST_RC :[NSNumber numberWithInt:lastReturnCode]]);

    } else if ([@"getLastCommandOutput" isEqualToString:call.method]) {

        NSString *lastCommandOutput = [MobileFFmpeg getLastCommandOutput];
        result([OpenCameraPlugin toStringDictionary:KEY_LAST_COMMAND_OUTPUT :lastCommandOutput]);
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            result([OpenCameraPlugin toMediaInformationDictionary:mediaInformation]);
        });
...
- (void)emitLogMessage:(NSDictionary*)logMessage{
    _eventSink([OpenCameraPlugin toStringDictionary:EVENT_LOG withDictionary:logMessage]);
}

- (void)emitStatistics:(Statistics*)statistics{
    NSDictionary *dictionary = [OpenCameraPlugin toStatisticsDictionary:statistics];
    _eventSink([OpenCameraPlugin toStringDictionary:EVENT_STAT withDictionary:dictionary]);
}
    ...
    if (mediaInformation != nil) {
        ...
        if (streams != nil && ([streams count] > 0)) {
            ...
            for (int i=0; i < [streams count]; i++) {
                ...
                [array addObject: [OpenCameraPlugin toStreamInformationDictionary:streamInformation]];
            }
